  ...
  return (
    <>
      ...
      <div className="bg-background text-on-background min-h-screen flex flex-col font-body-md antialiased overflow-x-hidden selection:bg-primary-container selection:text-on-primary relative">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
            <div className="glass-panel max-w-md w-full rounded-2xl p-8 border border-[#39FF14]/40 shadow-[0_0_60px_rgba(57,255,20,0.25)] flex flex-col items-center text-center relative overflow-hidden">
              ...
              <button
                ...
                className="w-full bg-primary-container text-on-primary py-4 rounded-xl font-headline-md text-body-lg uppercase tracking-widest shadow-[0px_10px_30px_rgba(57,255,20,0.2)] hover:shadow-[0px_10px_40px_rgba(57,255,20,0.35)] hover:bg-primary-fixed active:scale-[0.98] transition-all duration-200 cursor-pointer font-bold"
              >
        ...
        {/* Floating Role Selection Modal */}
        {showRoleSelectModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="glass-panel max-w-lg w-full rounded-3xl p-8 border border-[#39FF14]/40 shadow-[0_0_60px_rgba(57,255,20,0.25)] flex flex-col items-center text-center relative overflow-hidden">
              <div className="absolute -top-16 -left-16 w-40 h-40 bg-[#39FF14]/20 rounded-full blur-3xl pointer-events-none"></div>
              <div className="absolute -bottom-16 -right-16 w-40 h-40 bg-blue-500/20 rounded-full blur-3xl pointer-events-none"></div>

              <h3 className="text-3xl font-extrabold text-white tracking-wide mb-2 font-['Syne'] uppercase text-[#39FF14]">
                Selecciona tu Perfil
              </h3>
              <p className="text-gray-400 text-sm mb-8 font-['Inter']">
                ¿Cómo deseas ingresar a la plataforma de Match Pro hoy?
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full mb-6 font-['Inter']">
                {/* Jugador Card */}
                <button
                  type="button"
                  onClick={() => {
                    localStorage.setItem('matchProRole', 'jugador');
                    window.location.href = '/';
                  }}
                  className="bg-gray-800/50 hover:bg-[#39FF14]/10 border border-gray-700 hover:border-[#39FF14] rounded-2xl p-6 flex flex-col items-center text-center transition-all group cursor-pointer shadow-lg hover:shadow-[0_0_30px_rgba(57,255,20,0.2)]"
                >
                  <div className="w-16 h-16 rounded-full bg-[#39FF14]/20 border border-[#39FF14]/40 flex items-center justify-center text-[#39FF14] mb-4 group-hover:scale-110 transition-transform">
                    <span className="material-symbols-outlined text-3xl">sports_soccer</span>
                  </div>
                  <h4 className="text-white font-bold text-lg mb-2 font-['Syne'] group-hover:text-[#39FF14] transition-colors">Perfil de Jugador</h4>
                  <p className="text-gray-400 text-xs leading-relaxed">Reserva canchas, únete a partidos abiertos y revisa tus estadísticas.</p>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => {
                    localStorage.setItem('matchProRole', 'arrendador');
                    window.location.href = '/arrendador';
                  }}
                  className="bg-gray-800/50 hover:bg-blue-500/10 border border-gray-700 hover:border-blue-50 rounded-2xl p-6 flex flex-col items-center text-center transition-all group cursor-pointer shadow-lg hover:shadow-[0_0_30px_rgba(59,130,246,0.2)]"
                >
                  <div className="w-16 h-16 rounded-full bg-blue-500/20 border border-blue-500/40 flex items-center justify-center text-blue-400 mb-4 group-hover:scale-110 transition-transform">
                    <span className="material-symbols-outlined text-3xl">domain</span>
                  </div>
                  <h4 className="text-white font-bold text-lg mb-2 font-['Syne'] group-hover:text-blue-400 transition-colors">Perfil de Arrendador</h4>
                  <p className="text-gray-400 text-xs leading-relaxed">Gestiona tus complejos deportivos, canchas, horarios y reservas recibidas.</p>
                </button>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 flex flex-col items-center justify-center w-full max-w-md mx-auto px-container-padding pb-xl z-10 relative">
          ...
          <div className="w-full bg-surface-container-high rounded-xl p-xs flex mb-xl shadow-lg border border-white/5">
            <button 
              ...
              className={`flex-1 py-md rounded-lg font-label-mono text-label-mono transition-all cursor-pointer ${mode === 'login' ? 'bg-surface text-primary-container shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-white/10' : 'text-on-surface-variant hover:text-on-surface'}`}
            >
              Ingresar
            </button>
            <button 
              ...
              className={`flex-1 py-md rounded-lg font-label-mono text-label-mono transition-all cursor-pointer ${mode === 'register' ? 'bg-surface text-primary-container shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-white/10' : 'text-on-surface-variant hover:text-on-surface'}`}
            >
              Registrarme
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-lg">
            {error && (
              <div className="bg-error-container text-on-error-container p-3 rounded-xl text-center text-sm border border-error animate-in fade-in duration-300">
                {error}
              </div>
            )}
            ...
            {mode === 'register' && (
              <div className="space-y-sm animate-in fade-in duration-300">
                <label className="block text-on-surface-variant font-label-mono text-label-mono ml-sm" htmlFor="fullName">Nombre Completo</label>
            ...
            {mode !== 'forgot_password' && (
              <div className="space-y-sm animate-in fade-in duration-300">
                <label className="block text-on-surface-variant font-label-mono text-label-mono ml-sm" htmlFor="password">
                ...
                <div className="relative neon-glow-focus rounded-xl transition-shadow duration-300">
                  ...
                  <button 
                    ...
                    className="absolute inset-y-0 right-0 pr-md flex items-center text-on-surface-variant hover:text-primary-container transition-colors cursor-pointer"
                  >
            ...
            {mode === 'login' && (
              <div className="flex justify-end pt-xs">
                <button 
                  ...
                  className="text-on-surface-variant hover:text-primary-container font-label-mono text-label-mono transition-colors text-[12px] cursor-pointer"
                >
          ...
          <div className="flex justify-center">
            <button type="button" className="glass-panel w-[72px] h-[72px] rounded-full flex items-center justify-center text-primary-container hover:bg-surface-container transition-colors shadow-lg active:scale-95 group cursor-pointer">
